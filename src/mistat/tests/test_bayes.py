...
class TestBayes(unittest.TestCase):
    ...
    def test_updateGammaMixture(self):
        mixture = bayes.Mixture(
            probabilities=[0.5, 0.5],
            distributions=[
                bayes.GammaDistribution(shape=1, scale=1),
                bayes.GammaDistribution(shape=15, scale=2),
            ]
        )
        data = {'y': [5], 't': [1]}
        result = bayes.updateGammaMixture(mixture, data)
        assert result.distributions[0] == bayes.GammaDistribution(shape=6, scale=2)
        assert result.distributions[1] == bayes.GammaDistribution(shape=20, scale=3)
        assert result.probabilities[0] == pytest.approx(0.1250978)
        assert result.probabilities[1] == pytest.approx(0.8749022)

        data = {'y': [5, 3], 't': [1, 2]}
        result = bayes.updateGammaMixture(mixture, data)
        assert result.distributions[0] == bayes.GammaDistribution(shape=9, scale=4)
        assert result.distributions[1] == bayes.GammaDistribution(shape=23, scale=5)
        ...
        mixture = bayes.Mixture(
            ...
            distributions=[
                bayes.GammaDistribution(shape=1, scale=1),
                bayes.GammaDistribution(shape=15, scale=2),
                bayes.GammaDistribution(shape=10, scale=3),
            ]
        )
        result = bayes.updateGammaMixture(mixture, data)
